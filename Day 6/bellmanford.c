/*  Implementation of Bellman-Ford algorithm using adjacency list
...
int main()
{
    ...
    link_insert(&A, b, 1); link_insert(&A, c, 10); link_insert(&A, d, 4);
    link_insert(&B, a, 1); link_insert(&B, e, 1);
    link_insert(&C, a, 10); link_insert(&C, d, 4);
    link_insert(&D, c, 4); link_insert(&D, e, 1); link_insert(&D, a, 4);
    link_insert(&E, b, 1); link_insert(&E, d, 1);
    ...
    G->Adj = Adj;
  	...
  	Edge edges[] = {{a,b,1}, {b,a,1}, {d,a,4},{a,d,4}, {d,c,4}, {c,d,4}, {c,a,10}, {a,c,10}, {e,b,1}, {b,e,1}, {e,d,1}, {d,e,1}};
    ...
    {	    	
    	for(int j=0; j<12; j++)
    	{
    		Vertex *u = edges[j].v1;
    		Vertex *v = edges[j].v2;
            if( v->key > u->key + w(G->Adj, u->name, v->name))
            {
                v->parent = u;
                v->key = w(G->Adj, u->name, v->name) + u->key;
            }
        }    
    ...
    for(int i=0; i<5; i++)
    {
        printf("%s %d\n", vertices[i]->name, vertices[i]->key);
    }
}
